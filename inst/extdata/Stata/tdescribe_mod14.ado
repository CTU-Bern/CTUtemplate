...
* Modifications by A. Limacher

* Line 18: rename tdescribe to tdescribe_mod
* Line 2110 and 2141: Kruskal-Wallis chi-squared test with ties (chi2_adj) -> for the two-group case, this equals to the Wilcoxon ranksum test
...
program define tdescribe_mod14
version 14
...
if `a'>=2 & r(min)!=-99 & "`Pdrop'"!="on"{
	...
	foreach a of local continuous{ //													continuous variables of independent subgroups are compared with oneway anova
		...
		if `c'>0{
			...
			loc d: dis %6.3f chi2tail(`b',r(chi2_adj))
		}
		else{
