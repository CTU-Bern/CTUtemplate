...
{phang}
{bf:btable_format} {hline 2} Formating descriptive tables generated by {manhelp btable R:btable}
...
{synopt: {opth nametot(string)}}name of the column with all entries, default is "Total"{p_end}
...
{synopt: {opth inset(string)}}inset of category labels, default is "    " {p_end}
{synopt: {opth inset_row(string)}}inset of label in row design, default is " - " {p_end}
{synopt: {opth parse(string)}}parsing character for descriptives, effect measures or tests, default is "|" {p_end}
		
{syntab:Descriptives}
{synopt:{cmdab:des:criptive(}{it:{help btable_format##descspec:descspec}}{cmd:)}}type and format 
...
{synopt:{cmdab:eff:ect(}{it:{help btable_format##effectspec:effectspec}}{cmd:)}}type and format
...
{synopt:{cmdab:maxdec(}{it:{help btable_format##maxdecspec:maxdecspec}}{cmd:)}}the maximal 
	number of decimal places if digits_type is sigdig{p_end}
{synopt:{opth catd:igits0(numlist)}}forces specifc values to have 0 decimal places, only for categorical variables{p_end}
...
{pstd}
The descriptives (e.g. mean or median) and their format are selected 
...
P-values are only available if {it:groupvar} used with {cmd:btable} had at least two levels.
	The test used is selected via {cmd:test}.
	
{pstd}	
	The number of decimal places or significant digits (depending on option {cmd:digits_type})
		is controlled via {cmd:digits}, {cmd:digits_effect} and {cmd:p_digits}.
...
{phang}
{opth using filename} filename with results from {cmd: btable}{p_end}
...
{opt drop:(colspec)} specifies columns to drop; {it:colspec} 
	...
	the information about the applied descriptive statistics, effects and tests (info).
...
{phang}{opt digits(digitsspec)} specifies the digits of the descriptive for a variable category 
	(i.e. continuous, categorical, count or time-to-event) or a specific variable defined by {varname} from the original datafile
...
{pmore}
where {it:varspec} is {cmdab:conti}, {cmdab:cat}, {cmdab:count} or {it:varname}, and # is the number of digits. {p_end}
...
{phang}{opt digits_effect(digitsspec)} specifies the digits of the group difference for 
...
{phang}{opt abbr:eviation} abbreviate group difference name in header and include footnote for explanation.
...
Defaults are {cmd:ttest} or {cmd:ranksum} for continuous variables 
	if the {cmd:ft_type} includes {it: mean} or {it:median}, respectively, 
...
{pmore}
where {it:varspec} is {cmdab:conti}, {cmdab:cat}, {cmd:count}, {cmd:tte} or {varname}, 
...
{marker maxdecspec}{...}	
{phang}{opt maxdec(maxdecspec)} specifies the maximal number of decimal places if {cmd:digits_type} is {cmd:sigdig}.
	Does not have an effect if {cmd:digits_type} is {cmd:decimals}. 
...
{pmore}
where {it:varspec} is {cmdab:conti}, {cmdab:cat}, {cmd:count}, {cmd:tte} or {varname}. The default is 2. {p_end}
...
{phang}{opth collapse(collapsespec)} specifies variables that should be collapsed according to their {varname} 
	...
	The option is only valid for categorical variables with one or two levels.{p_end}
...
{phang}{cmd:. btable_format using "excars", clear digits_type(sigdig) drop(total effect)}{p_end}

{phang}{cmd:. btable_format using "excars", clear digits_type(sigdig) digits(cat 2 conti 3) drop(total effect)}{p_end}

{phang}{cmd:. btable_format using "excars", clear  digits(price 4) drop(total effect)}{p_end}
