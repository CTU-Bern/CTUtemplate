...
if (strpos("`teslist'","ranksum")>0 & "`ranksum_exact'" != "") {
	cap which ranksumex
	if _rc {
		dis as error "ranksumex is required; type -net install st0297, from(http://www.stata-journal.com/software/sj13-2)- to obtain it"
		exit 499
		//dis "Installing auxiliary package ranksumex: "
		//net install st0297, from(http://www.stata-journal.com/software/sj13-2)
	}
}	
	...
	********************
	...
	if "`continuous'" != "" {
		...
		forvalues i = 1/`cwc'  {
			...
			if "`var'"=="`wi'" {
				...
				cap assert "`typei'"==""
				if _rc {
					dis "Variable `var' mentioned under categorical and continuous, default option used."
					local typei
					local stopt="yes"
				}
				else {
	...
	if "`count'"!="" & "`stopt'" != "yes"  {
		...
		if strpos("`count'","`var'")>0 {
			...
			cap assert "`typei'"=="" 
			if _rc {
				dis "Variable `var' mentioned under count and categorical or continuous, count option ignored."
			}
			else {
				local pcount = 0
	...
	if "`tte'"!="" & "`stopt'" != "yes"  {
		...
		if strpos("`tte'","`var'")>0 {
			cap assert "`typei'"==""  
			if _rc {
				dis "Variable `var' mentioned under tte and categorical, continuous or count, tte option ignored."
			}
			else {
				local ttcount = 0
		...
		*over all levels:
		...
		if "`by'"=="" {
			...
			if (`lc'>`nclimfish' | `nt'>`nlimfish') & "`fisher'"=="" &  strpos("`testlist2'","fisher")>0 {
				...
				dis "Fisher's exact test for `var' is omitted `outp'."
				dis "Use test(fisher) to use the test anyway, might be slow."
					...
					*alternative confidence intervals for rd, rr and or
					...
					if "`orci'"!="" & "`orci'"!="woolf" &  strpos("`effectlist'","or")>0  {
						...
						if "`orci'"=="exact" {
							...
							if _rc {
								dis "Error using exact confidence intervals for odds ratio for `var':`lbl'"
							}
							local or_lci=r(lb_or)
							local or_uci=r(ub_or)
						}
						if "`orci'"=="cornfield" {
							...
							if _rc {
								dis "Error using cornfield confidence intervals for odds ratio for `var':`lbl'"
							}
							local or_lci=r(lb_or)
							local or_uci=r(ub_or)
						}
						if "`orci'"=="gart" {
							...
							if _rc {
								dis "Error using gart confidence intervals for odds ratio for `var':`lbl'"
							}
						...
						if "`orci'"=="agresti" {
							...
							if _rc {
								dis "Error using agresti confidence intervals for odds ratio for `var':`lbl'"
							}
				...
				if `n0t'!=0 & `n1t'!=0  { 
					...
					if strpos("`testlist2'","ranksum")>0  {		
						...
						if "`ranksum_exact'" == "" {
							...
							qui ranksumex `var' if `touse3', by(`grrc')
							if "`r(p)'"=="" {
								cap local npar_p=2*(1-normal(abs(`r(z)')))
							} 
							else {
								cap local npar_p=`r(p)'
							}
						}
					}
					else {
			...
			if `gc'==2 {
				...
				if strpos("`testlist2'","poisson")==0  ///
					...
					*alternative Poisson p-values:
					if "`poisson_test'"!="" & "`poisson_test'"!="exact" & strpos("`testlist2'","poisson")>0 {				
						if ("`poisson_test'"=="oim") {
							if "`p_poi_oim'"== "" {
								cap poisson `var' `grrc' if `touse3', level(`level') exposure(`timevar') vce(oim)
								if _rc {
	...
	*************************
	...
	preserve	
		...
		if "`by'"=="" {
			...
			if "`trunctime'"=="" {
				local trunctime = `mic'
				local text1= "Truncation time is missing, the minimum of the largest observed event time"
...
********************************
*Function for asymptotic CI for Mann-Whitney statistic according to R function wmwTest from asht
*after Fay2018
********************
...
if "`vmethod'"!="" & !inlist("`vmethod'","LAPH","PO") {
	dis "Invalid vmethod, default (LAPH) used"
	local vmeth 0
}
...
if  "`alternative'"!="" & !inlist("`alternative'","two-sided","greater","less") {
	dis "Invalid alternative, only two-sided, greater or less allowed. Default two-sided used."
	local alt 0
}
